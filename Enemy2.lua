...
local level=display.newText("Level:2", display.contentCenterX-10,display.contentCenterY-200,
native.systemFont,20)
level:setTextColor(255, 255, 255)
  ...
  local jankenOpt =
  {
   frames = {
     ...
     {x= 153, y= 62, width= 23, height= 31 }, -- 7. enemy2_shake1
     {x= 178, y= 62, width= 23, height= 31 }, -- 8. enemy2_shake2
     {x= 236, y= 62, width= 15, height= 31 }, -- 9. enemy2_set
  ...
  if (enemy_score~=2 and alex_score~=2) then
    ...
    if(alex_score<enemy_score) then
      ...
      local function button4(event)
        composer.removeScene( "Enemy2", options )
    ...
    elseif(alex_score>enemy_score) then
     ...
     local function button3(event)
      composer.removeScene( "Enemy2", options )
      composer.gotoScene( "Enemy3" , options )
      nextBtn.isVisible=false
     end
