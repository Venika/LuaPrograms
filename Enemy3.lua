...
local nextBtn= display.newImage("again.png")
...
function scene:create( event )
   ...
   local level=display.newText("Level:3", display.contentCenterX-10,display.contentCenterY-200,
   native.systemFont,20)
     ...
     if (enemy_score~=2 and alex_score~=2) then
       ...
       if(alex_score<enemy_score) then
         ...
         local function button4(event)
           composer.removeScene( "Enemy3", options )
...
alex_score = 0;
   ...
   function play()
      ...
      --cal t = timer.performWithDelay (delay, progressView.isVisible=false, 1);
      -- Create the widget
